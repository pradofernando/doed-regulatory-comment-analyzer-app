...
.\deploy.ps1 -RegulationsGovApiKey "your-api-key" -DocumentId "YOUR_DOCUMENT_ID" -EnableMethodologySearch -SearchLocation centralus

# Optional: use regional Standard embedding quota when GlobalStandard quota is unavailable
.\deploy.ps1 -RegulationsGovApiKey "your-api-key" -DocumentId "YOUR_DOCUMENT_ID" -EnableMethodologySearch -EmbeddingSkuName Standard
```
...
These options configure the optional services only; `-EnableMethodologySearch`
must also be supplied to provision Search and embeddings. GPT-5.5 remains the
independent, explicitly pinned model for the four agents.
