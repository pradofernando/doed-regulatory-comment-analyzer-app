...
async def main():
    ...
    # ── Phase 5: Human-readable run summary ───────────────────────────────────
    logging.info("\n>>> PHASE 5: Generating human-readable run summary...")
    ...
    with open(f"output_5_run_summary_{timestamp}.txt", "w", encoding="utf-8") as f:
        f.write(run_summary)
    logging.info(f"Saved  output_5_run_summary_{timestamp}.txt")

    upload_to_blob(run_summary,
                   f"4_summary/run_summary_{timestamp}.txt", storage_account)
